...
**B3** — это код третьей технической вертикали группы B: **Transactional &
Operational Data**, то есть транзакционные и операционные хранилища данных.
Трек учит не просто писать SQL, а проектировать правила хранения, получать
корректные результаты, сохранять инварианты при конкуренции, безопасно изменять
схему и проверять эксплуатационные гарантии хранилища. Место B3 среди остальных
вертикалей показано в [общей карте треков](../../program/track-map.md).

## Содержание: 11 модулей B3

Модуль — крупный профессиональный результат. Ссылка в названии ведёт к его
границам, развиваемым capabilities и исключениям. Статус маршрута показывает,
есть ли уже полный набор учебных материалов, а не только blueprint ожидаемых
результатов.

| Модуль | Что осваивается | Учебный маршрут |
|---|---|---|
| [Конкурентные транзакции](modules.md#preserve-invariants) | Сохранять общий инвариант, объяснять аномалии изоляции и диагностировать конфликты | Пока blueprint |
| [Реляционная модель и ограничения](modules.md#relational-model-schema) | Выражать структуру и правила данных отношениями, ключами и ограничениями | Пока blueprint |
| [Корректность SQL-результата](modules.md#sql-querying) | Управлять гранулярностью результата, соединениями, агрегациями и оконными вычислениями | [Готовый срез SQ01](slices/sql-result-correctness/README.md) |
| [Сохранность, WAL и crash recovery](modules.md#storage-wal-recovery) | Связывать журнал предзаписи, подтверждение записи, checkpoint и восстановление после сбоя | Пока blueprint |
| [Индексы и оптимизатор](modules.md#indexes-optimizer) | Читать план запроса и выбирать путь доступа с учётом цены чтения и записи | Пока blueprint |
| [Эволюция схемы](modules.md#schema-evolution) | Менять схему при сосуществующих readers/writers, выполнять backfill и проверять данные | [Готовый срез SE01](slices/compatible-orders-amount-migration/README.md) |
| [Репликация и высокая доступность](modules.md#replication-ha) | Проверять видимость и сохранность данных при репликации и смене узла | Пока blueprint |
| [Партиционирование и масштабирование](modules.md#partitioning-scaling) | Выбирать разбиение данных и находить пределы store-level масштабирования | Пока blueprint |
| [Резервное копирование и восстановление](modules.md#backup-restore-dr) | Проектировать цепочку копий и доказывать восстановление данных в заданную точку | Пока blueprint |
| [Безопасность и обслуживание](modules.md#security-operations) | Ограничивать доступ и диагностировать состояние, мешающее эксплуатации store | Пока blueprint |
| [Выбор операционного хранилища](modules.md#operational-nonrelational-stores) | Сопоставлять модель store с данными и запросами и проверять его технический контракт | Пока blueprint |

Если нужен готовый материал для самостоятельного прохождения, начинайте с
[каталога учебных срезов B3](slices/). Полный перечень capabilities и критериев
